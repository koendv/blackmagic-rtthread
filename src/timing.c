...
void platform_timeout_set(platform_timeout_s *const timeout, uint32_t ms)
{
	if (ms < SYSTICKMS)
		ms = SYSTICKMS;
	timeout->time = platform_time_ms() + ms;
}

bool platform_timeout_is_expired(const platform_timeout_s *const timeout)
{
	 ...
	 * overflows to a low timeout->time value, if we simply compare with `<`, we will
