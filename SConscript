...
LOCAL_CPPDEFINES += ['PLATFORM_RTTHREAD=1']
LOCAL_CPPDEFINES += ['ENABLE_MEMWATCH=1']
LOCAL_CPPDEFINES += ['ENABLE_MTB=1']
LOCAL_CPPDEFINES += ['ADVERTISE_NOACKMODE=1']

# src/target/meson.build lists defines needed
...
LOCAL_CPPDEFINES += [ 'CONFIG_LATTICE=1' ]
LOCAL_CPPDEFINES += [ 'CONFIG_LATTICE_ECP5_DECODE=1' ]
...
src += Glob(join('src', 'target', '*.c'), exclude=[join('src', 'target', 'swdptap_generic.c'), join('src', 'target', 'jtagtap_generic.c'), join('src', 'target', 'semihosting.c')])
